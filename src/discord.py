...
class Context:
  ...
  async def purge(self, amount):
    async with aiohttp.ClientSession() as session:
      while True:
        async with session.get(f'{self.api}/{self.context["d"]["channel_id"]}/messages', headers = self.headers) as msg:
          if msg.status == 429:
            msg = await msg.json()
            ...
            break
      for m in msg:
        if m['author']['id'] == self.info()['id']:
          while True:
            async with session.delete(f'{self.api}/channels/{self.context["d"]["channel_id"]}/messages/{m["id"]}', headers = self.headers) as delete:
              if delete.status == 429:
                delete = await delete.json()
                asyncio.sleep(delete['retry_after'])
              else:
